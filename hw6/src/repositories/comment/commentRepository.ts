...
class CommentRepository extends Repository<Comment> implements ICommentRepository {
    ...
    public async setLike(queryRunner: Repository<Comment>, comment: IComment, id: number): Promise<UpdateResult> {
        return queryRunner.update({ id }, { like: comment.like + 1 });
    }

    public async setDislike(queryRunner: Repository<Comment>, comment: IComment, id: number): Promise<UpdateResult> {
